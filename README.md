...
### データベース設計・正規化・リレーションシップ

本アプリケーションのデータベースは、工場の各工程・品質管理を効率的に記録・分析できるよう、正規化とリレーションシップを意識して設計されています。

#### 1. 正規化のポイント

- **第1正規形（1NF）**  
  各テーブルのカラムは分割できない値（原子値）で構成されています。  
  例：検査記録の「重量」「厚み」「形状評価」などは個別カラムで管理。

- **第2正規形（2NF）**  
  主キーに対して部分的な依存がないよう設計されています。  
  例：各記録テーブルは `id` を主キーとし、検査日時や担当者などの情報を持ちます。

- **第3正規形（3NF）**  
  非キー属性が他の非キー属性に依存しないよう分離されています。  
  例：担当者名やロット番号などは、各テーブルで参照しやすい形で管理。

#### 2. 主なテーブルと役割

- **resin_checks**：樹脂供給監視記録
- **temperature_records**：温度・基準管理記録
- **preform_quality_checks**：プリフォーム品質確認記録
- **molding_inspections**：成型工程監視記録
- **cooling_records**：冷却時間ログ記録
- **final_inspections**：最終検査記録
- **process_traceability**：各工程のトレーサビリティ統合記録

#### 3. テーブル間のリレーションシップ

- 各工程ごとの記録テーブル（例：preform_quality_checks, molding_inspections など）は、  
  ロット番号やサンプル番号、製品番号などで工程をまたいだデータの紐付けが可能です。
- **process_traceability** テーブルは、各工程の記録を「ロット番号」「工程段階」「参照テーブル名」「参照レコードID」で統合的に管理し、トレーサビリティ（追跡性）を実現します。
- これにより、1つのロットに関する全工程の履歴を一元的に追跡・分析できます。

#### 4. ER図イメージ（概略）
  ![ER図](Untitled%20diagram%20_%20Mermaid%20Chart-2025-07-13-062238.png)

#### 5. 拡張性・保守性

- 各工程ごとに独立したテーブル設計としつつ、**process_traceability**で全体を統合することで、  
  データの重複を排除しつつ、工程追加や仕様変更にも柔軟に対応できる構造となっています。
